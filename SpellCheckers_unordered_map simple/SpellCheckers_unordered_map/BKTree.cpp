...
void BKTree::RecursiveSearch(string searchString, int tolerance, Node * current, unordered_map<string, double> * resultSet) {
	int distance = LevenshteinDistance(current->word, searchString), upper = distance + tolerance, lower = distance - tolerance;

	if (distance <= tolerance)
		resultSet->insert(make_pair(current->word, distance));
